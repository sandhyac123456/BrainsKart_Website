  ...
  return (
    <>
      ...
      <div className="px-4 py-8 pb-16 bg-gray-50 min-h-screen">
        ...
        {loading ? (
          <p className="text-red-500 text-2xl sm:text-3xl text-center">
            Loading...
          </p>
        ) : items.length > 0 ? (
           ...
           <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
            ...
            {items.map((item) => (
              ...
              >
               {/* Image */}
<div className="w-full h-52 sm:h-64 min-h-72 flex items-center justify-center">  <img
    src={item.image}
    alt={item.name}
  ...
  />
                ...
                <div className="p-3 sm:p-4 flex flex-col items-center gap-2 sm:gap-3">
                  ...
                  <Link
                    to={`/item/${item._id}`}
                    className="w-full text-center bg-[#bb742e] text-white py-2 rounded-md font-semibold text-sm sm:text-base hover:bg-[#c99b64] transition"
                  >
                    ADD TO CART
                  </Link>
